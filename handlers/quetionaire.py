...
async def form_state_about_player(message: Message, state: FSMContext):
    ...
    if message.voice:
        ...
        await message.answer(f"Заполните анкету до конца: если Вы хотите что-то изменить, "
                             f"Вы сможете отправить новую анкету только после заполнения этой")
        return
...
async def form_state_what_to_do(message: Message, state: FSMContext):
    ...
    if message.voice:
        ...
        await message.answer(f"Маловато как-то. Напиши более развёрнуто ну пж🙏")
        return
    else:
        data = message.text
        is_text = True
...
async def form_state_nick_nick(message: Message, state: FSMContext):
    ...
    if not message.text:
        ...
        if message.video:
            await message.answer(f"В видео я как ник увижу? Просто напиши ник")
        else:
